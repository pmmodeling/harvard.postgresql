...
addresses$Latitude = addresses$lat 
addresses$Longitude = addresses$lng
...
conn <- dbConnect(drv = "PostgreSQL", host = "localhost", dbname = "bc", user = "postgres", password = yourpassword)
...
for (i in 1:length(allshapefiles)){

shapefile = readOGR(dsn = paste0(allshapefiles[i]), 
                    layer = paste0(sapply(strsplit(allshapefiles[i],".",fixed = TRUE),"[[",1)), 
                    stringsAsFactors = FALSE, pointDropZ = T, 
	p4s = "+init=epsg:5070 +proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0")
pgInsert(conn, name = c("public", paste0(sapply(strsplit(allshapefiles[i],".",fixed = TRUE),"[[",1))), data.obj = shapefile, 
         overwrite = TRUE)
...
tablenames = gsub('.shp','',allshapefiles)
...
# remove quotation marks from  id variable

dbGetQuery(bc_psql$con,"update _addresses set id = trim(id, '\"')")


# -- create a spatial index - ignore warnings


for (i in 1:length(tablenames)){
  
postgiscommand = paste0('CREATE INDEX ', tablenames[i],'_gix ON ', tablenames[i],' USING GIST (geom);')
dbGetQuery(bc_psql$con,postgiscommand) 
...
dbGetQuery(bc_psql$con, "create table step01 as (SELECT _addresses.id, _addresses.geom, lat, lng, elev_m, dvhi_1km, dvlo_1km, imp1km
...
dbGetQuery(bc_psql$con, "alter table step02 add column aadt character varying, add column disttoge10k double precision;")
...
dbGetQuery(bc_psql$con, 'update step02 set fullname = sub.railtype, disttorail = sub.rail_dis from (
SELECT DISTINCT ON (a.id) a.id, bg.railtype, ST_Distance(a.geom, bg.geom) as rail_dis
	FROM step02 a
...
SELECT DISTINCT ON (buff.id) buff.id,  coalesce(sum(ST_Length(ST_Intersection(st_buffer(buff.geom, 100), _hpms13.geom))), 0) as sum
FROM step02 as buff left join _hpms13 on ST_Intersects(st_buffer(buff.geom, 100), _hpms13.geom) group by buff.id) as sub where step02.id = sub.id;")
...
dbGetQuery(bc_psql$con, 'create table step23 as (SELECT _hpms13.gid, buff.id, ST_length(ST_Intersection(st_buffer(buff.geom, 100), _hpms13.geom)) as len, ST_Intersection(st_buffer(buff.geom, 100), _hpms13.geom), _hpms13.aadt as aadt 
					FROM _hpms13, _addresses as buff WHERE ST_Intersects(st_buffer(buff.geom, 100), _hpms13.geom));')
...
SELECT DISTINCT ON (buff.id) buff.id,  coalesce(sum(ST_Length(ST_Intersection(st_buffer(buff.geom, 200), _hpms13.geom))), 0) as sum
FROM step02 as buff left join _hpms13 on ST_Intersects(st_buffer(buff.geom, 200), _hpms13.geom) group by buff.id) as sub where step02.id = sub.id;")
...
saveRDS(gis_psql, paste0(mypath,'/gisoutput.rds'))


meteo_psql  = tbl(bc_psql, "step18")
meteo_psql = as.data.frame(meteo_psql)
saveRDS(meteo_psql, paste0(mypath,'/meteo.rds'))


adtxlth  = tbl(bc_psql, "step23")
adtxlth = as.data.frame(adtxlth)

saveRDS(adtxlth, paste0(mypath,'/adtxlth.rds'))
